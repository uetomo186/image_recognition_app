...
class VisionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isTest)
      drawRect(
          Rect.fromLTWH(size.width / 2 - 10, size.height / 2 - 10, 20, 20));

    // 16:10 (Up-down black) or 17:9 (Left-right black)
    scale =
        dw / dh < 16.0 / 9.0 ? dw / cameraSize.width : dh / cameraSize.height;
    _canvas.scale(scale);
    ...
    if (vision.type == VisionType.FACE) {
      ...
      for (Face f in vision.faces) {
        ...
        if (f.smilingProbability != null) {
          drawText(Offset(r.left, r.top),
              'smil ${(f.smilingProbability! * 100.0).toInt()}', _fontSize);
        }
        if (f.headEulerAngleX != null) {
          drawText(Offset(r.left, r.top + _fontHeight),
              'x ${(f.headEulerAngleX! * 100.0).toInt()}', _fontSize);
        }
        if (f.headEulerAngleY != null) {
          drawText(Offset(r.left, r.top + _fontHeight * 2),
              'y ${(f.headEulerAngleY! * 100.0).toInt()}', _fontSize);
        }
        if (f.headEulerAngleZ != null) {
          drawText(Offset(r.left, r.top + _fontHeight * 3),
              'z ${(f.headEulerAngleZ! * 100.0).toInt()}', _fontSize);
        }
      ...
      if (vision.text == null) return;
      for (TextBlock b in vision.text!.blocks) {
        drawRect(b.boundingBox);
        drawText(
            Offset(b.boundingBox.left, b.boundingBox.top), b.text, _fontSize);
      }
    } else if (vision.type == VisionType.IMAGE) {
      if (vision.labels.isEmpty) return;
      int i = 0;
      for (ImageLabel label in vision.labels) {
        String s = "${(label.confidence * 100.0).toInt()} ${label.label}";
        drawText(
            Offset(_textLeft, _textTop + _fontHeight * (i++)), s, _fontSize);
      }
    } else if (vision.type == VisionType.BARCODE) {
      if (vision.barcodes.isEmpty) return;
      int i = 0;
      ...
      debugPrint('SELFIE=w= ${width.toString()} ${height.toString()}');
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
          if (confidences[(y * width) + x] < 0.5) {
            _paint.style = PaintingStyle.fill;
            _canvas.drawCircle(
                Offset(x.toDouble() * dx, y.toDouble() * dy), 1.0, p);
          }
        }
      }
    } else if (vision.type == VisionType.OBJECT) {
      if (vision.objects.isEmpty) return;
      for (DetectedObject b in vision.objects) {
        drawRect(b.boundingBox);
        int i = 0;
        Rect r = b.boundingBox;
        drawText(Offset(r.left, r.top + _fontHeight * (i++)),
            'id ${b.trackingId}', _fontSize);
        List<Label> ls = b.labels;
        for (Label s in ls) {
          drawText(
              Offset(r.left, r.top + _fontHeight * (i++)), s.text, _fontSize);
        }
      }
    } else if (vision.type == VisionType.TENSOR) {
      if (vision.results.isEmpty) return;
      int i = 0;
      for (TfResult res in vision.results) {
        for (TfOutput out in res.outputs) {
          String s = "${(out.score).toInt()} ${out.label}";
          drawText(
              Offset(_textLeft, _textTop + _fontHeight * (i++)), s, _fontSize);
          if (i > 5) break;
        }
      ...
      for (TfResult res in vision.results) {
        drawRect(res.rect);
        String s = "${(res.outputs[0].score).toInt()} ${res.outputs[0].label}";
        drawText(
            Offset(res.rect.left, res.rect.top - _fontHeight), s, _fontSize);
      }
    } else if (vision.type == VisionType.POSE) {
      if (vision.poses.isEmpty) return;
      vision.poses.forEach((pose) {
        ...
        drawPoseLine(
            lms, PoseLandmarkType.leftKnee, PoseLandmarkType.leftAnkle);
        drawPoseLine(
            lms, PoseLandmarkType.leftAnkle, PoseLandmarkType.leftHeel);
        drawPoseLine(
            lms, PoseLandmarkType.leftHeel, PoseLandmarkType.leftFootIndex);
        ...
        drawPoseLine(
            lms, PoseLandmarkType.leftShoulder, PoseLandmarkType.leftElbow);
        drawPoseLine(
            lms, PoseLandmarkType.leftElbow, PoseLandmarkType.leftWrist);
        ...
        drawPoseLine(
            lms, PoseLandmarkType.leftWrist, PoseLandmarkType.leftThumb);
        drawPoseLine(
            lms, PoseLandmarkType.leftWrist, PoseLandmarkType.leftIndex);
        drawPoseLine(
            lms, PoseLandmarkType.leftWrist, PoseLandmarkType.leftPinky);
        ...
        drawPoseLine(
            lms, PoseLandmarkType.rightHip, PoseLandmarkType.rightKnee);
        drawPoseLine(
            lms, PoseLandmarkType.rightKnee, PoseLandmarkType.rightAnkle);
        drawPoseLine(
            lms, PoseLandmarkType.rightAnkle, PoseLandmarkType.rightHeel);
        drawPoseLine(
            lms, PoseLandmarkType.rightHeel, PoseLandmarkType.rightFootIndex);
        ...
        drawPoseLine(
            lms, PoseLandmarkType.rightShoulder, PoseLandmarkType.rightElbow);
        drawPoseLine(
            lms, PoseLandmarkType.rightElbow, PoseLandmarkType.rightWrist);
        ...
        drawPoseLine(
            lms, PoseLandmarkType.rightWrist, PoseLandmarkType.rightThumb);
        drawPoseLine(
            lms, PoseLandmarkType.rightWrist, PoseLandmarkType.rightIndex);
        drawPoseLine(
            lms, PoseLandmarkType.rightWrist, PoseLandmarkType.rightPinky);
        ...
        drawPoseLine(
            lms, PoseLandmarkType.rightMouth, PoseLandmarkType.leftMouth);
      });
    }
  }
  ...
  drawContour(Face f, FaceContourType type) {
    ...
    if (c != null) {
      ...
      c.points.asMap().forEach((i, pt) {
        i == 0
            ? path.moveTo(pt.x.toDouble(), pt.y.toDouble())
  ...
  drawLandmark(Face f, FaceLandmarkType type) {
    ...
    if (l != null) {
      _paint.style = PaintingStyle.fill;
      _canvas.drawCircle(
          Offset(l.position.x.toDouble(), l.position.y.toDouble()),
          6.0,
          _paint);
    }
  }
  ...
  drawPoseLine(Map<PoseLandmarkType, PoseLandmark> landmarks,
  ...
  drawText(Offset offset, String text, double size) {
    TextSpan span = TextSpan(
      text: " $text ",
      style: TextStyle(
          color: COLOR1, backgroundColor: Colors.black54, fontSize: size),
    );
  ...
  _test(Canvas canvas) {
    ...
    canvas.drawRect(const Rect.fromLTWH(0, 0, 100, 100), p);
    ...
    drawText(const Offset(100, 100), "ABCDEFG", 24);
  }
